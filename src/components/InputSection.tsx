  ...
  <div className={styles.container}>
    <div className={styles.grid}>
      <div className={styles.column}>
        ...
        <InputField id="valorEntrada" label="Valor da Entrada (R$)" value={inputs.valorEntrada} onChange={handleInputChange} placeholder="100000" error={errorEntrada} helpText="Exigir uma entrada de, no mínimo, 20% do valor do imóvel é uma prática padrão na maioria dos financiamentos bancários." />
        <InputField id="valorSubsidio" label="Subsídio (R$)" value={inputs.valorSubsidio} onChange={handleInputChange} placeholder="55000" helpText="O valor do subsídio não é o mesmo para todos. Ele depende principalmente da sua renda familiar mensal, que determina em qual faixa do programa Minha Casa, Minha Vida(2025), você se encaixa.(valor max: 55 mil)" />
        <InputField id="jurosAnual" label="Taxa de Juros Anual (%)" value={inputs.jurosAnual} onChange={handleInputChange} placeholder="7" helpText="A taxa de juros do Minha Casa, Minha Vida também está diretamente ligada à faixa de renda da família. Assim como no subsídio, a lógica é a mesma: o governo oferece um benefício maior para quem tem menor renda. (4% ate 12%, dependendo da faixa ou banco que ira financiar.) " />
      ...
      <div className={styles.column}>
        ...
        <InputField id="reajusteAnual" label="Reajuste Anual (%)" value={inputs.reajusteAnual} onChange={handleInputChange} placeholder="6" helpText="Percentual de aumento anual do aluguel.Pode varia de região para região." />
      </div>
    </div>
